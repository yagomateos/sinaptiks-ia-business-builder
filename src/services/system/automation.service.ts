...
export const automationService = {
  ...
  async remove(automation: Automation): Promise<void> {
    // Si n8n no confirma el borrado, no se borra la fila local: perder
    // `n8n_workflow_id` dejaría el workflow real huérfano en n8n para
    // siempre, sin ninguna referencia en la app que permita encontrarlo y
    // limpiarlo más adelante.
    if (automation.n8n_workflow_id) {
      await n8nService.deleteWorkflow(automation.n8n_workflow_id)
    }
