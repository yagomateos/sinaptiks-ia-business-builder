...
- **Formulario de conexión en el marketplace** (2026-09-11): `whatsapp.service.ts` + `ConnectWhatsAppDialog` (mismo patrón que Telegram) piden `phone_number_id` y token de acceso, y los validan contra la Graph API vía `connect-whatsapp` antes de guardar nada. Ya no está oculto (`HIDDEN_FOR_NOW` volvió a quedar vacío) — igual que Stripe, un negocio sin credenciales de Meta ve el error real de la API en vez de una conexión fingida.
- **Pendiente por credenciales/trámite**: falta que la plataforma tenga `WHATSAPP_VERIFY_TOKEN`/`WHATSAPP_APP_SECRET`, y cada negocio necesita su propio número verificado en Meta Business Suite — eso sigue siendo un trámite externo que ningún cambio de código puede completar.
