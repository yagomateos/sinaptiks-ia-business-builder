...
- **`listAvailability` ofrece varias franjas por hueco libre** (revisión profesional, 2026-09-11): antes solo devolvía la primera franja de cada tramo libre continuo (un día entero libre solo ofrecía "9:00", no 10:00/11:00/...). Ahora genera franjas seguidas de `durationMinutes` hasta llenar el hueco. **Nota real encontrada de paso**: `listAvailability` no lo llama nada en el flujo de citas hoy — `agendar_cita` (la acción que sí crea el evento real) no comprueba disponibilidad antes de reservar, así que dos peticiones para la misma hora crearían dos eventos solapados sin aviso. Queda fuera de alcance a propósito (decisión pendiente: ¿rechazar, avisar al equipo, o dejarlo así?).
