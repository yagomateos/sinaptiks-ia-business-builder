...
- **`listAvailability` ofrece varias franjas por hueco libre** (revisión profesional, 2026-09-11): antes solo devolvía la primera franja de cada tramo libre continuo (un día entero libre solo ofrecía "9:00", no 10:00/11:00/...). Ahora genera franjas seguidas de `durationMinutes` hasta llenar el hueco.
- **`agendar_cita` ya evita citas duplicadas** (2026-09-12): nuevo método `CalendarProvider.isAvailable(startsAt, endsAt)` (freeBusy acotado exactamente a la hora pedida, no a la cuadrícula de `listAvailability`). `createCalendarAppointment` lo comprueba antes de crear el evento — si el hueco ya está ocupado, no crea un evento duplicado: marca la cita como `error` con la nota "Ese hueco ya está ocupado en el calendario" y lanza un error con el mensaje exacto para el equipo (reutiliza el aviso de "automatización falló" que ya existía, sin duplicar lógica de notificación). Antes, dos solicitudes para la misma hora habrían creado dos eventos solapados sin que nadie se enterase.
- **`registrar_solicitud_cita` ya reserva de verdad en el calendario** (2026-09-12): antes, la herramienta que el agente llama en plena conversación solo avisaba al equipo y siempre respondía "el equipo te contactará" — nunca tocaba Google Calendar, aunque estuviera conectado y el hueco libre. Se extrajo `bookCalendarAppointment` a `_shared/appointment-booking.ts` (comparte la comprobación de disponibilidad + creación de evento con `agendar_cita`, antes duplicado) y ahora, con fecha válida + email, la conversación reserva de verdad: el agente le confirma la cita real al cliente al momento en vez de prometer un seguimiento que tarda; si el hueco está ocupado, le pide otra hora en la misma conversación; sin calendario conectado o sin esos datos, sigue cayendo en "solicitud registrada" como antes. 4 tests nuevos para `bookCalendarAppointment` (sin calendario / confirmada / conflicto / error de Google).
